...
if [ $# -ne 3 ]; then
    ...
    echo "LANG: 'ru' — русский, 'en' — английский"
...
if [[ "$LANG" != "ru" && "$LANG" != "en" ]]; then
    echo "❌ Неподдерживаемый язык: '$LANG'. Используйте 'ru' или 'en'."
    exit 1
fi
...
echo "✅ Бот успешно установлен! Язык: $( [[ "$LANG" == "ru" ]] && echo "Русский" || echo "Английский" )"
